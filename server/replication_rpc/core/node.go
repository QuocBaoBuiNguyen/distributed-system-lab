...
ping:
	log.Info().Msgf("[Node]: HealthCheckLeader: %s", leaderID)
...
func (node *Node) RegisterWithPeers() {
	...
	log.Info().Msgf("[Node]: Node %s - shard %s is aware of peers %v", node.ID, node.ShardID, ports)
	nodeAddressByID := make(map[string]string)

	for _, port := range ports {
		nodeID := node.GetNodeID(strings.TrimPrefix(port, ":")) 
		nodeAddressByID[nodeID] = port                          
	}
