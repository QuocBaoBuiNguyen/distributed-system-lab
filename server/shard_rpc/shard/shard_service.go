...
func (s *ShardService) MigrateData(req *common.MigrationRequestArgs, reply *string) error {
	...
	log.Info().Msgf("Migrating data from buckets %v to %s with hash range %v (start: %d, end: %d)", buckets, req.TargetAddress, req.HashRange, req.HashRange.Start, req.HashRange.End)
	...
	for _, bucket := range buckets {
		...
		log.Info().Msgf("Migrated data from bucket, target: %s, bucket: %s", req.TargetAddress, bucket)
...
func (s *ShardService) NotifyShardPeerUpdate(req *common.NotifyNodesInShardArgs, res *string) error {
	log.Info().Msgf("Received updated peers %v", req.Ports)
	for _, port := range req.Ports {
		nodeID := s.Node.GetNodeID(strings.TrimPrefix(port, ":"))
		if !slices.Contains(s.Node.Peers.ToIDs(), nodeID) && nodeID != s.Node.ID {
			log.Info().Msgf("New node %s is added to peers %s", nodeID, s.Node.Peers.ToIDs())
			s.Node.Peers.Add(nodeID, port, s.Node.Connect(port))
		}
	}
	log.Info().Msgf("Updated peers %s", s.Node.Peers.ToIDs())
	return nil
}
